...
## Declare new variable (column) here because R doesn't like doing that inside a for() loop
lut_species$tsn <- ""
...
getTSN <- function(x) {
  for (i in 1:nrow(x)) {
    if (nchar(x[i,ncol(x)]) == 0) {
      lut_species[i,]$tsn <<- get_tsn(x[i,]$scientificName_clean, searchtype = "sciname", ask = TRUE, verbose = TRUE)   
    }}
}
...
getTAX <- function(x) {
  for (i in 1:nrow(x)) {
    if (length(out) <= i) {
      ...
      message(paste("Looking up information for TSN:", tsn))
    }}
}
...
##***************************************************************************************************************##
##    Warning!!! If this script breaks, chances are it is be because "Division" or "Subspecies" categories       ##
##               exist in your dataset. Add the necessary fields (in proper order) to the list below.            ##               
##***************************************************************************************************************##
